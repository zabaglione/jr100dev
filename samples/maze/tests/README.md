...
* 確認手順:
  1. `make -C samples/maze/tests build/maze_init_test.prg`
...
* 確認手順:
  1. `make -C samples/maze/tests build/maze_step_test.prg`
  2. エミュレーターでロードし `A=USR($0300)` を実行。
...
* 仕様:
  ...
  * 確認手順:
  1. `make -C samples/maze/tests build/maze_stack_test.prg`
  2. エミュレーターでロードし `A=USR($0300)` を実行。
...
* 確認手順:
  1. `make -C samples/maze/tests build/maze_neighbors_test.prg`
  2. エミュレーターでロードし `A=USR($0300)` を実行。
...
* 確認手順:
  1. `make -C samples/maze/tests build/maze_wall_mark_test.prg`
  2. エミュレーターでロードし `A=USR($0300)` を実行。
...
* 確認手順:
  1. `make -C samples/maze/tests build/maze_carve_test.prg`
  2. エミュレーターでロードし `A=USR($0300)` を実行。
...
## 8. クリーニング
```
make -C samples/maze/tests clean
```
