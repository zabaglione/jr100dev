...
重力を変えると、球が1マスずつ転がって止まるまでを表示します。複数の球が移動する順序も追えます。演出中の追加入力は受け付けません。
...
バージョン 1.6.0。開始番地 `$0300`、ゲーム本体と定数は 11,040 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
