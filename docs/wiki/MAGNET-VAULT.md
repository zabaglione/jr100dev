...
バージョン 1.0.0。開始番地 `$0300`、ゲーム本体と定数は 7,512 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
