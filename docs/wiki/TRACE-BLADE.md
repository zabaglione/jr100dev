...
**スピードフォント**（右へ傾く太線）を採用。タイトル／説明用に16文字、ゲーム用に3文字を割り当てています。ゲーム中の対象は `>+/` です。空きPCG枠だけを使い、残りの文字は通常フォントで表示します。数字を変更する作品では0〜9を一式で揃えています。
...
開発環境を用意して `make -C games/trace_blade` を実行します。出力は `build/trace-blade.prg`、開始番地は `$0300`。本体・定数は8,491 bytesで、状態、画面、BASICへ戻すための保存領域、512 bytesのスタックを含めて標準16KB内です。PCGは32文字です。
