...
白い石板の刻印が階段状に移り変わる構図です。ロゴにも石の刻みを入れています。
...
バージョン 1.4.0。開始番地 `$0300`、ゲーム本体と定数は 10,687 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
