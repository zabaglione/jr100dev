...
リポジトリの開発環境を用意して `make -C games/abyss_signal` を実行します。出力は `build/abyss-signal.prg`、開始番地は `$0300`。ゲーム本体と定数は9,684 bytes、画面・状態・保存領域・512 bytesのスタックを含めて標準16KB内です。PCGは場面ごとに32文字を使います。
