...
バージョン：1.0.0。開始番地は `$0300`、ゲーム本体と定数は11,515 bytes、PCGは31文字です。画面バッファー・地形・状態・復帰用保存領域・512 bytesのスタックを含めて標準RAM 16KB内に収めています。
...
地形1,000種の接続・配置、8方向入力、品物・戦闘・視界・罠、描画と命令をC++エミュレーターで検査しています。保存入力だけを再生し、EASYの5階・NORMALの10階・HARDの20階で遺物取得を確認しました。移行前とゲームの機械語は同一です。
