...
操作方法や得点ルールはタイトル・ヘルプにまとめる。本編には盤面、現在の選択、残り体力、得点、行先や予告など判断に必要な情報を置く。常設のキー一覧や攻略説明は出さない。命中、失敗理由、獲得点、クリアなど、その場で起きた結果は短く表示する。選択式メニューの項目名は操作対象として残す。
...
最大3本の列車は同じ俯瞰の車体PCG4文字を共有し、1文字ずつ中間位置を通って進む。個体別の行先・急行の猶予は近くの文字と一覧で示す。駅看板は目的駅だけを点灯させ、駅の荷下ろし待ちと信号停止を盤面上へ表示する。到着時は加点または失敗理由、SEと短い停止を挟む。通常便と急行の得点規則・キー一覧はヘルプに置く。

## SAND RESCUEとGATE RUNNERの再構成

SAND RESCUEは6面で水を共有し、芽・葉・収穫物の絵を給水に合わせて切り替える。水門は閉・半開・開、流れは連続した短い水列、乾いた水路で失う水は散る形と音で示す。収穫量と残り水を結果に残してから次の畑へ進む。

GATE RUNNERはタイトルと同じセミグラフィックスの遠景に、道路・壁・穴・梁を投影する。主人公は横27位置を1文字ずつ移動し、走行2形と跳躍1形で4文字のPCGを更新する。障害物は接近して大きくなり、通過後の位置まで描く。壁・穴・梁の被害を別の短い表示と停止で示す。

道路の描画・背景の展開・通常時の画面転送はゲーム専用のMB8861H処理で軽量化した。開始時の段階表示では共通のフィルターを使い、通常時は画面コードを直接転送する。音と入力の走査は転送の行間で継続し、RAM・PCGの制約は変えない。
...
STAR LANCE 4.0.0では、編隊を1文字ずつ動かし、自弾・敵弾・撃破の発光と破片を通常の進行中に更新します。左右の長押しと射撃の同時入力を読み、自弾や撃破演出が戦闘全体を止めないようにしました。自機の被弾と開始・結果の場面では、状況を確認する間を保ちます。HULL・ENEMIES・HEATと短い通知は下端の2行、結果は最下行へまとめ、敵と自機の間には案内文を置きません。
