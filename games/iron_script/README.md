...
**計器盤フォント**（角張った太線）を採用。タイトル／説明用に16文字、ゲーム用に16文字を割り当てています。ゲーム中の対象は `0123456789BATERY` です。空きPCG枠だけを使い、残りの文字は通常フォントで表示します。数字を変更する作品では0〜9を一式で揃えています。
...
バージョン 1.2.0。開始番地 `$0300`、ゲーム本体と定数は 8,990 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
