...
未調査の土地を側面のある土のブロック、調査済みの土地を低い床として描きました。主人公と方位表示を手前に置いています。 向きに応じて1体4文字のPCGを書き換えます。 移動時には半マスの中間位置とSEを挟みます。 岩を避けて方角を読みながら進みます。未探索の地面と岩を描き分け、掘る途中の動作と、発見の光・音を加えました。
...
岩を避けて方角を読みながら進みます。未探索の地面と岩を描き分け、掘る途中の動作と、発見の光・音を加えました。被弾や結果を確認する間は、追加入力で表示を飛ばせません。
...
岩を避けて方角を読みながら進みます。未探索の地面と岩を描き分け、掘る途中の動作と、発見の光・音を加えました。
...
バージョン 2.0.1。開始番地 `$0300`、ゲーム本体と定数は 7,655 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
