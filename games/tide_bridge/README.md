...
道がつながると、主人公が進行方向を向き、マスの中間を通って門まで歩きます。到着後は両手を上げて2回跳ね、喜びの効果音に続いてクリアのジングルが鳴ります。演出中の追加入力は受け付けません。
...
バージョン 1.6.0。開始番地 `$0300`、ゲーム本体と定数は 7,766 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
