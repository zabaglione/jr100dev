...
[![操作を進めた場面・クリックで動画](images/demo-play.png)](https://zabaglione.github.io/pyjr100emu/gameplay.html?game=prism-trace)
...
鏡を回した後、光が通るマスを順に表示します。反射して進む経路を追えます。演出中の追加入力は受け付けません。
...
鏡を回した後、光が通るマスを順に表示します。反射して進む経路を追えます。演出が終わってから次のキーを押してください。
...
バージョン 1.5.0。開始番地 `$0300`、ゲーム本体と定数は 6,323 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
