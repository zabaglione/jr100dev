...
どちらの種類の壁が障害物になるかを切り替え、ダイヤへ進みます。10面それぞれで経路と切り替える場所を考えます。

![タイトル](images/title.png)

## 操作と遊び方

WASDで移動、RETURNで壁の規則を切り替えます。切り替えは1面12回までです。

方向キーはキーボードまたはパッド、RETURNはパッドのボタンでも操作できます。SPACEでこの面をやり直し、CTRL+CでBASICへ戻ります。

2種類の壁、現在の規則、切り替え回数を表示します。
...
バージョン 1.0.0。開始番地 `$0300`、ゲーム本体と定数は 7,231 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
