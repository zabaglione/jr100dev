...
[Wiki](https://github.com/zabaglione/jr100dev/wiki/CORNER-CROWN) · [プレイ](https://zabaglione.github.io/pyjr100emu/?game=corner-crown)
...
[![操作を進めた場面・クリックで動画](images/demo-play.png)](https://zabaglione.github.io/pyjr100emu/gameplay.html?game=corner-crown)

![最初の目標を達成した場面](images/demo-clear.png)

## タイトルのデザイン

角を強調した盤と大きな王冠を左右に分け、CORNERとCROWNをそれぞれに対応させます。

## 画面の奥行き

明るい駒と中が暗い駒に縁の厚みを付けました。盤は正方格子のまま、角と挟み取りの範囲を判別しやすくしています。 石返しは専用4文字を使い、細くなる面、側面、開く反対面の5枚の中間コマを表示します。
...
挟んだ石は一枚ずつ、面が細くなって側面を見せ、反対の面が開く順に回転します。白から黒、黒から白の両方に途中の形と石返しの音があります。演出中の追加入力は受け付けません。
...
バージョン 1.5.0。開始番地 `$0300`、ゲーム本体と定数は 7,930 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
