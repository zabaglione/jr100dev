...
挟んだ石は一枚ずつ、面が細くなって側面を見せ、反対の面が開く順に回転します。白から黒、黒から白の両方に途中の形と石返しの音があります。被弾や結果を確認する間は、追加入力で表示を飛ばせません。
...
バージョン 1.5.1。開始番地 `$0300`、ゲーム本体と定数は 7,928 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
