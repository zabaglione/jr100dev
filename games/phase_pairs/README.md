...
1枚目を選ぶと、隣接して合計10になる候補の枠がすべて点滅します。数字を読める状態で次の選択先を示します。合計10になる2枚は中央へ寄り合い、大きな「10」に合体してから光を散らして消えます。選択音と合体音で一組ずつの成立を確認できます。誤った組は盤面を残し、隣接していないのか、合計が10ではないのかを表示します。演出中の追加入力は受け付けません。
