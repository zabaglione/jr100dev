...
**[音付きプレイ動画を見る（約36秒）](https://zabaglione.github.io/pyjr100emu/gameplay.html?game=cargo-balance)**
...
船に甲板・船腹・水面の段差を付け、積荷には斜めの側面を描きました。荷重の数字と左右の釣り合いは従来の位置で確認できます。 次の2つの荷重を予告します。積荷が船倉へ落下し、重い側へ船体が傾きます。左右の重さに加え、危険な偏りまでの距離を目盛で示します。
...
次の2つの荷重を予告します。積荷が船倉へ落下し、重い側へ船体が傾きます。左右の重さに加え、危険な偏りまでの距離を目盛で示します。被弾や結果を確認する間は、追加入力で表示を飛ばせません。
...
次の2つの荷重を予告します。積荷が船倉へ落下し、重い側へ船体が傾きます。左右の重さに加え、危険な偏りまでの距離を目盛で示します。
...
バージョン 1.6.1。開始番地 `$0300`、ゲーム本体と定数は 7,376 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
