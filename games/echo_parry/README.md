...
対戦場に奥へ向かう床線と手前の段差を付け、剣士と盾に厚みを加えました。上段・下段と反撃の合図は従来の位置に表示します。 敵の攻撃、盾での受け止め、反撃の飛翔、命中を順に描きます。攻撃の高さは単純な交互から8手のパターンへ変えました。
...
敵の攻撃、盾での受け止め、反撃の飛翔、命中を順に描きます。攻撃の高さは単純な交互から8手のパターンへ変えました。被弾や結果を確認する間は、追加入力で表示を飛ばせません。
...
バージョン 1.6.0。開始番地 `$0300`、ゲーム本体と定数は 7,092 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
