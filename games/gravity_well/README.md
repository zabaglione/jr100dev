...
金属の盤、丸い球の明暗、くぼんだ受け皿で高さの違いを出しました。球と任意ルーンは丸みと十字の印で区別できます。 移動時には半マスの中間位置とSEを挟みます。
...
重力を変えると、球がマスの中間を通って転がり、止まるまでを表示します。複数の球が移動する順序も音とともに追えます。被弾や結果を確認する間は、追加入力で表示を飛ばせません。
...
バージョン 1.7.1。開始番地 `$0300`、ゲーム本体と定数は 10,952 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
