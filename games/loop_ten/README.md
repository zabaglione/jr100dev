...
迷宮の石壁、門、封印に厚みを付け、主人公には接地影を加えました。時間ゲージと危険な床の記号は見やすさを優先しています。 移動の中間位置、封印の反応、危険地形への接触、手動巻き戻しに途中の絵と音があります。移動演出の時間も10秒の制限に含まれます。
...
移動の中間位置、封印の反応、危険地形への接触、手動巻き戻しに途中の絵と音があります。移動演出の時間も10秒の制限に含まれます。被弾や結果を確認する間は、追加入力で表示を飛ばせません。
...
開発環境を用意して `make -C games/loop_ten` を実行します。出力は `build/loop-ten.prg`、開始番地は `$0300`。本体と定数は7,939 bytes。画面・状態・保存領域・512 bytesのスタックを含めて標準16KB内、PCGは32文字です。
