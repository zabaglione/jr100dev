...
[![操作を進めた場面・クリックで動画](images/demo-play.png)](https://zabaglione.github.io/pyjr100emu/gameplay.html?game=loop-ten)
...
開発環境を用意して `make -C games/loop_ten` を実行します。出力は `build/loop-ten.prg`、開始番地は `$0300`。本体と定数は7,580 bytes。画面・状態・保存領域・512 bytesのスタックを含めて標準16KB内、PCGは32文字です。
