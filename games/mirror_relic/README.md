...
[![操作を進めた場面・クリックで動画](images/demo-play.png)](https://zabaglione.github.io/pyjr100emu/gameplay.html?game=mirror-relic)
...
壁に斜めの反射面、扉に厚み、遺物に面の明暗を付けました。回転後の通路と人物の位置は正方格子で示します。 向きに応じて1体4文字のPCGを書き換えます。 移動時には半マスの中間位置とSEを挟みます。
...
探索者の向きと、マスの中間を通る移動を表示します。鏡を使う前後の位置を音とともに確認できます。演出中の追加入力は受け付けません。
...
バージョン 1.5.0。開始番地 `$0300`、ゲーム本体と定数は 6,685 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
