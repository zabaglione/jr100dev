...
着陸船を窓と脚のある機体にし、遠くの山と手前の月面を分けて描きました。接地判定に使う着陸台は明確な水平線で表示します。 降下の中間位置と噴射炎を描きます。着陸台の上へ速度0〜2で降りると、機体の停止を確認する間と着陸音を挟んでクリアします。
...
降下の中間位置と噴射炎を描きます。着陸台の上へ速度0〜2で降りると、機体の停止を確認する間と着陸音を挟んでクリアします。被弾や結果を確認する間は、追加入力で表示を飛ばせません。
...
バージョン 1.6.1。開始番地 `$0300`、ゲーム本体と定数は 6,758 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
