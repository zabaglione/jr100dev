...
MOTION = {
    "dice-relic": "攻撃では主人公が踏み込んで光弾を飛ばし、命中した敵が点滅します。防御では盾が広がり、敵の攻撃から防いだ量と残るダメージを表示します。被弾すると主人公が点滅して後ずさりし、回復では光が上昇します。HPや防御値の変化を確認する間を挟み、敵の消滅と勝利報酬まで順番に表示します。",
    "chain-suit": "交換する札は細くなって側面を見せ、新しい札として開きます。採点時は役を作る札の印が点滅し、役名・今回の点数・合計点を残して効果音と約2秒の間を挟んでから、次の手札へ進みます。",
    ...
    "prism-trace": "鏡を回すと、光がつながった線として伸びていきます。鏡の面で線が折れ曲がり、反射音とともに受光器までの経路を追えます。",
    "tide-bridge": "道がつながると、主人公が進行方向を向き、マスの中間を通って門まで歩きます。到着後は両手を上げて2回跳ね、喜びの効果音に続いてクリアのジングルが鳴ります。",
    ...
    "five-forge": "石が落下し、着地してつぶれ、元の形に戻る四段階の動きとSEを付けました。自分と相手の着手を一手ずつ表示し、五連の石だけを3回点滅させてから結果を表示します。",
    "orbit-draft": "選んだ札は列の空き枠まで落下し、行・列の回転では途中の位置を通って反対側へ回り込みます。三連以上が揃うとセミグラフィックスの光が三段階に広がり、消去音とともに消えます。残った札が落ちて再び揃うと連鎖音が鳴り、得点と回転回数が増えます。",
