...
頭と胴を丸みのある形にし、光沢と下側の影を付けました。盤面の厚みを加えつつ、通れるマスは正方格子で示します。 向きに応じて1体4文字のPCGを書き換えます。 頭だけでなく胴体もマスの中間を通ります。食べると光と音が出て体が伸び、画面端と自分の体への衝突をそれぞれ説明します。
...
頭だけでなく胴体もマスの中間を通ります。食べると光と音が出て体が伸び、画面端と自分の体への衝突をそれぞれ説明します。被弾や結果を確認する間は、追加入力で表示を飛ばせません。
...
バージョン 1.6.1。開始番地 `$0300`、ゲーム本体と定数は 7,551 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
