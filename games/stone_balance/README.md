...
石を上面・前面・側面のあるブロックにし、三つの山を別々の棚に配置しました。個数と選択中の山を優先して読めます。 自分と相手が取った石を、一つずつ盤面から移動させます。相手の応答を確認する間があり、最後の石を取った側が勝ちます。
...
自分と相手が取った石を、一つずつ盤面から移動させます。相手の応答を確認する間があり、最後の石を取った側が勝ちます。被弾や結果を確認する間は、追加入力で表示を飛ばせません。
...
バージョン 1.6.0。開始番地 `$0300`、ゲーム本体と定数は 7,273 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
