...
4つのダイヤルが回り、判定が1桁ずつ進みます。全桁一致では扉が幅を変える3段階の開閉を行い、開錠音と光からジングルへ続きます。
...
4つのダイヤルが回り、判定が1桁ずつ進みます。全桁一致では扉が幅を変える3段階の開閉を行い、開錠音と光からジングルへ続きます。被弾や結果を確認する間は、追加入力で表示を飛ばせません。
...
バージョン 2.1.0。開始番地 `$0300`、ゲーム本体と定数は 7,802 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
