...
堰を土の断面、作物を幹と影のある木にし、水を短い波線で区別しました。流れの分岐と水門の位置は正方格子を保っています。 水がマスの中間を通り、堰が途中の形を経て開きます。作物が必要な水を受け取ると光と音で示します。
...
水がマスの中間を通り、堰が途中の形を経て開きます。作物が必要な水を受け取ると光と音で示します。被弾や結果を確認する間は、追加入力で表示を飛ばせません。
...
バージョン 1.6.1。開始番地 `$0300`、ゲーム本体と定数は 8,119 bytes。画面・作業領域・復帰用の保存領域・512 bytesのスタックを含めて標準RAM 16KB内で動作します。PCGは32文字を場面ごとに切り替えます。
